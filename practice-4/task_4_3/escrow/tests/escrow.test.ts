...
describe("escrow", () => {
  ...
  test("Offer cancelled by Alice, vault and offer account are cleaned up", async () => {
    const getTokenBalance = getTokenBalanceOn(connection);
    ...
    // Verify state before the offer cancellation
    // Vault
    expect(await getTokenBalance(vaultAddress)).toEqual(new BN(10_000_000));
    // Maker
    expect(await getTokenBalance(aliceUsdcAccount)).toEqual(new BN(90_000_000));
    expect(await getTokenBalance(aliceWifAccount)).toEqual(new BN(5_000_000));
    // Taker
    ...
    // Verify state after the offer cancellation
    // Maker
